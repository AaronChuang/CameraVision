package com.vision.aaron.cameravision;
...
public class MainActivity extends AppCompatActivity {
  ...
  private static final String CLOUD_VISION_API_KEY = "AIzaSyA0j2Jwc7EPgYi4yq1NLkgVdB0STIbJpJ8";
    ...
    fab.setOnClickListener(
        view -> {
          ...
          builder
              ...
              .setPositiveButton(
                  R.string.dialog_select_gallery, (dialog, which) -> startGalleryChooser())
  ...
  public void startGalleryChooser() {
    if (PermissionUtils.requestPermission(
        this, GALLERY_PERMISSIONS_REQUEST, Manifest.permission.READ_EXTERNAL_STORAGE)) {
    ...
    switch (requestCode) {
      case CAMERA_PERMISSIONS_REQUEST:
        if (PermissionUtils.permissionGranted(
            requestCode, CAMERA_PERMISSIONS_REQUEST, grantResults)) {
          startCamera();
        }
        break;
      case GALLERY_PERMISSIONS_REQUEST:
        if (PermissionUtils.permissionGranted(
            requestCode, GALLERY_PERMISSIONS_REQUEST, grantResults)) {
          startGalleryChooser();
        }
        break;
    }
  ...
  private void callCloudVision(final Bitmap bitmap) {
    ...
    try {
      AsyncTask<Object, Void, String> textDetectionTask =
          new TextDetectionTask(this, prepareAnnotationRequest(bitmap));
        ...
        new ArrayList<AnnotateImageRequest>() {
          {
            ...
            annotateImageRequest.setFeatures(
  ...
  private static class TextDetectionTask extends AsyncTask<Object, Void, String> {
    ...
    protected String doInBackground(Object... params) {
      try {
        ...
        Log.d(TAG, "receive response:" + response.toString());
  ...
  private static String convertResponseToString(BatchAnnotateImagesResponse response) {
    StringBuilder message = new StringBuilder("解析:\n");

    List<EntityAnnotation> texts = response.getResponses().get(0).getTextAnnotations();
    if (texts != null) {
      int i = 1;
      for (EntityAnnotation text : texts) {
        message.append(String.format(Locale.CHINESE, "結果%d: %s", i, text.getDescription()));
        message.append("\n");
        i++;
      }
    } else {
